...
- **Deployment**: production is hosted on **Manus Autoscale** (`chalkpicks.pro`), with a Manus-provided cloud computer attached for background/data workloads. A migration to Railway was started and then reversed — the deploy machinery it added (`Dockerfile`, `railway.toml`, `railway.vars.json`, the Railway env-sync workflow, `docs/RAILWAY_*.md`) is gone, and `.github/workflows/` is down to `ci.yml`, so **there are no Railway deploy checks on PRs**. What survives is only a read-only status widget — `server/routers/railway.ts` and `client/src/components/RailwayStatusWidget.tsx` over `RAILWAY_API_TOKEN` — which degrades to empty when the token is unset.
