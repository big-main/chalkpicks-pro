...
- **Deployment**: production is hosted on **Manus Autoscale** (`chalkpicks.pro`), with a Manus-provided cloud computer attached for background/data workloads. A migration to Railway was started and then reversed — the in-repo deploy machinery it added (`Dockerfile`, `railway.toml`, `railway.vars.json`, the Railway env-sync workflow, `docs/RAILWAY_*.md`) is gone, and `.github/workflows/` is down to `ci.yml`. Railway is **not** where production runs. But the Railway GitHub App is still connected to the "Chalkpicks Pro" project, so `railway-app[bot]` still spins up a per-PR preview environment and comments on PRs — that comment is expected, and is not evidence that Railway is the deploy target. Also still present: a read-only status widget (`server/routers/railway.ts`, `client/src/components/RailwayStatusWidget.tsx`) over `RAILWAY_API_TOKEN`, which degrades to empty when the token is unset.
- **Background jobs run on Manus Heartbeat, not a local cron**: `server/_core/heartbeat.ts` registers jobs that call back into `/api/scheduled/*`, and the `server/handlers/*` read `x-manus-cron-task-uid` for logging. Registration goes through `BUILT_IN_FORGE_API_URL`/`BUILT_IN_FORGE_API_KEY` — the _same_ Manus Forge credentials `server/_core/llm.ts` uses for LLM calls. Repointing those at a plain OpenAI-compatible endpoint keeps picks generating but silently kills every scheduled job.
- **Canonical origin is the apex `https://chalkpicks.pro`** (no `www`), after the move off `chalkpicks.live` — which is 301'd in `server/_core/index.ts`. `SITE_URL` in `shared/seo-routes.ts` is the source of truth; prefer importing it over adding another hardcoded origin literal.
