...
- **Deployment**: production is hosted on **Manus** (`chalkpicks.pro`), with a Manus-provided cloud computer attached for background/data workloads. Railway is also in active use — `server/routers/railway.ts` and `client/src/components/RailwayStatusWidget.tsx` talk to a real Railway project via `RAILWAY_API_TOKEN` — so Railway deploy checks on PRs are real signal, not noise to ignore.
