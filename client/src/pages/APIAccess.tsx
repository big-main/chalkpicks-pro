  ...
  const generateMutation = trpc.apiKeys.generate.useMutation({
    onSuccess: data => {
      ...
      toast.success("API Key Generated", {
        description: `Key created with ${TIER_LIMITS[data.tier] ?? 100} req/day limit.`,
      });
    },
  ...
  const endpoints = [
    {
      method: "GET",
      path: "/api/v1/picks",
      description: "Get picks by sport and date",
      example: "/api/v1/picks?sport=NFL&date=2026-07-16",
    },
    {
      method: "GET",
      path: "/api/v1/odds/ev",
      description: "Get positive EV opportunities",
      example: "/api/v1/odds/ev?sport=basketball_nba&minEV=3",
    },
    {
      method: "GET",
      path: "/api/v1/odds/devig",
      description: "Devig any set of odds",
      example: "/api/v1/odds/devig?odds=-110,-110",
    },
    {
      method: "GET",
      path: "/api/v1/elo/ratings",
      description: "Get Elo power ratings by sport",
      example: "/api/v1/elo/ratings?sport=nfl",
    },
    {
      method: "GET",
      path: "/api/v1/leaderboard",
      description: "Get live leaderboard rankings",
      example: "/api/v1/leaderboard?limit=50",
    },
    {
      method: "POST",
      path: "/api/v1/bets/track",
      description: "Track a bet and get CLV updates",
      example: "POST body: { sport, team, odds, stake }",
    },
  ];
    ...
    <div className="min-h-screen bg-background text-foreground px-4 py-10 max-w-5xl mx-auto">
      ...
      >
        ...
        <p className="text-muted-foreground">
          Integrate ChalkPicks data into your own tools. Generate API keys,
          monitor usage, and explore endpoints.
        </p>
      </motion.div>
              ...
              {keys.map(key => (
                ...
                >
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <Badge
                      className={`text-xs ${TIER_COLORS[key.tier] ?? TIER_COLORS.basic}`}
                    >
                      {key.tier}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    <span>
                      {key.requestsToday}/{TIER_LIMITS[key.tier] ?? 100} today
                    </span>
        ...
        {[
          {
            ...
            label: "Free Tier",
            limit: "100 req/day",
            desc: "Basic picks & odds data",
          },
          {
            tier: "pro",
            label: "Pro Tier",
            limit: "1,000 req/day",
            desc: "EV, CLV, devig endpoints",
          },
          {
            tier: "enterprise",
            label: "Enterprise",
            limit: "10,000 req/day",
            desc: "Full API access + Elo",
          },
        ].map(t => (
      ...
      <Card className="border border-border mb-8">
        ...
        <CardContent>
          ...
          <pre className="bg-muted/30 rounded-lg p-4 text-sm font-mono text-foreground overflow-x-auto">
            {`curl https://chalkpicks.pro/api/v1/picks \\
  ...
  -H "Content-Type: application/json"`}
          ...
          {newlyGenerated ? (
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  value={
                    showKey
                      ? newlyGenerated
                      : newlyGenerated.slice(0, 12) + "•".repeat(20)
                  }
            ...
            <div className="space-y-4">
              ...
              <DialogFooter>
                ...
                <Button
                  onClick={() =>
                    generateMutation.mutate({ name: newKeyName || "Default" })
                  }
      ...
      >
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            This action is permanent. Any applications using this key will lose
            access immediately.
          </p>
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() =>
                revokeId !== null && revokeMutation.mutate({ id: revokeId })
              }
