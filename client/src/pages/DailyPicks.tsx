      ...
      <main className="flex-grow container mx-auto px-4 py-8 max-w-5xl mt-20">
        <div className="mb-10 text-center">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Every AI pick we've published, by date — wins, losses, and all. Full
            transparency, no cherry-picking.
          </p>
        </div>
        ...
        {isLoading && (
          <div className="text-center py-20 text-muted-foreground">
            Loading picks archive...
          </div>
        )}

        {!isLoading && (!data || data.days.length === 0) && (
          <div className="text-center py-20 text-muted-foreground">
            No picks found for this filter. Check out{" "}
            <Link href="/picks" style={{ color: "#00ff88" }}>
              today's picks
            </Link>
            .
          </div>
              ...
              <section key={day.date}>
                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                  ...
                  {(wins > 0 || losses > 0) && (
                    ...
                    >
                      <TrendingUp className="w-3 h-3 mr-1" /> {wins}-{losses}{" "}
                      Record
                    </Badge>
                  )}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {day.picks.map(pick => {
                    const rs =
                      RESULT_STYLES[pick.result ?? "pending"] ??
        ...
        <div className="mt-16 mb-8">
          ...
          >
            ...
            <p className="text-muted-foreground mb-6 max-w-xl mx-auto">
              Premium members see every pick with full AI analysis, confidence
              scores, and edge ratings — updated daily.
            </p>
            {hasFullAccess ? (
              <Link href="/picks">
                <button
                ...
                >
                  VIEW TODAY'S PICKS <ChevronRight className="w-4 h-4" />
                </button>
              </Link>
            ) : (
              <Link href="/pricing">
