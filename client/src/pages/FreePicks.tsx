...
import { BreadcrumbJsonLd, FaqJsonLd, SportsEventJsonLd } from "@/components/seo/schema-jsonld";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { trpc } from "@/lib/trpc";
import { Zap, Target, TrendingUp, ChevronRight, Lock, CheckCircle2, Star } from "lucide-react";
      ...
      <div className="relative z-10 container pt-28 pb-20 max-w-4xl">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-4">
            Free Sports Betting Picks<br />
            <span className="text-[#39ff14]">Powered by AI</span>
          </h1>
          <p className="text-muted-foreground max-w-xl mx-auto text-lg">
            Get free AI-generated sports picks every day. NFL, NBA, MLB, NHL, and more — no credit card required.
          </p>
        ...
        <NeonCard className="p-6 mb-10 text-center">
          {submitted ? (
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-foreground font-bold">You're in! Check your inbox for today's free picks.</p>
            </div>
          ) : (
            <>
              <h2 className="text-lg font-bold text-foreground mb-2">Get Free Picks in Your Inbox</h2>
              <p className="text-muted-foreground text-sm mb-4">Daily AI picks delivered every morning. No spam, unsubscribe anytime.</p>
        ...
        <div className="mb-10">
          ...
          {picks.length > 0 ? (
                ...
                <NeonCard key={pick.id} className="p-5">
                  <div className="flex items-start justify-between mb-3 flex-wrap gap-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge className="text-xs bg-[#39ff14]/10 text-[#39ff14] border-[#39ff14]/20">FREE</Badge>
                      </div>
                      <p className="text-base font-bold text-foreground">
                        {pick.awayTeam} @ {pick.homeTeam}
                      </p>
                      <p className="text-sm text-muted-foreground mt-0.5">
                        Pick: <span className="text-foreground font-medium">{pick.recommendation}</span>
                      </p>
                    </div>
                    <Link href={`/picks/${pick.id}`}>
                      <Button size="sm" variant="outline" className="border-[#39ff14]/30 text-[#39ff14] hover:bg-[#39ff14]/10">
                        View Pick
                      </Button>
                    </Link>
                  </div>
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-xs text-muted-foreground mb-1">
                      ...
                      <span className="text-foreground">{pick.confidenceScore || 72}%</span>
                    </div>
                  ...
                  <div className="mt-3 relative">
                    <div className="blur-sm select-none text-xs text-muted-foreground">
                      Edge: +4.2% | Odds: -110 | Sharp money: 67% | AI Analysis: The {pick.homeTeam} have covered in 8 of their last 10 home games...
                    </div>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <Link href="/pricing">
                        <Button size="sm" className="bg-[#39ff14] hover:bg-[#32e012] text-black font-bold text-xs">
            ...
            <div className="space-y-4">
              {[
                { sport: "NFL", away: "Buffalo Bills", home: "Kansas City Chiefs", rec: "Kansas City Chiefs -3.5", conf: 78 },
                { sport: "NBA", away: "Golden State Warriors", home: "Los Angeles Lakers", rec: "Over 228.5", conf: 71 },
                { sport: "MLB", away: "Boston Red Sox", home: "New York Yankees", rec: "New York Yankees ML", conf: 65 },
              ].map((pick, i) => (
                <NeonCard key={i} className="p-5">
                  <div className="flex items-start justify-between mb-3 flex-wrap gap-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <Badge className="text-xs bg-slate-700/50 text-slate-300 border-slate-600">{pick.sport}</Badge>
                        <Badge className="text-xs bg-[#39ff14]/10 text-[#39ff14] border-[#39ff14]/20">FREE</Badge>
                      </div>
                      <p className="text-base font-bold text-foreground">{pick.away} @ {pick.home}</p>
                      <p className="text-sm text-muted-foreground mt-0.5">Pick: <span className="text-foreground font-medium">{pick.rec}</span></p>
                    </div>
                    <Link href="/picks">
                      <Button size="sm" variant="outline" className="border-[#39ff14]/30 text-[#39ff14] hover:bg-[#39ff14]/10">
                        View Pick
                  ...
                  <div className="mt-3 relative">
                    <div className="blur-sm select-none text-xs text-muted-foreground">
                      Edge: +3.8% | Odds: -115 | Sharp money: 71% | AI Analysis: Strong value on this line based on recent form...
                    </div>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <Link href="/pricing">
                        <Button size="sm" className="bg-[#39ff14] hover:bg-[#32e012] text-black font-bold text-xs">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-10">
          {[
            { name: "Mike T.", text: "Been using ChalkPicks for 3 months. Up 22 units on NFL alone.", stars: 5 },
            { name: "Sarah K.", text: "The free picks alone beat what I was paying for at other services.", stars: 5 },
            { name: "James R.", text: "AI analysis is incredibly detailed. Worth every penny for Pro.", stars: 5 },
          ].map(({ name, text, stars }) => (
            <NeonCard key={name} className="p-4">
              ...
              <p className="text-xs text-muted-foreground mb-2 italic">"{text}"</p>
