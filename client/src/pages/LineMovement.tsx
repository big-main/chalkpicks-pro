          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Card className="bg-gray-900 border-red-500/30">
                <CardContent className="p-4 text-center">
                  <p className="text-2xl font-bold text-red-400">{data.sharpMoves}</p>
                  <p className="text-xs text-gray-400">Sharp Moves Detected</p>
                </CardContent>
              </Card>
              <Card className="bg-gray-900 border-gray-800">
                <CardContent className="p-4 text-center">
                  <p className="text-2xl font-bold text-yellow-400">${lineMutation.data?.balance?.toFixed(2)}</p>
            ...
            <div className="space-y-3">
              {data.movements.map((move, i) => (
                <Card key={i} className={`bg-gray-900 ${move.isSharpMove ? "border-red-500/40" : "border-gray-800"}`}>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className={`p-2 rounded-lg ${move.direction === "up" ? "bg-emerald-500/10" : "bg-red-500/10"}`}>
                          {move.direction === "up" ? (
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <Badge variant="outline" className={
                          Math.abs(move.movement) > 2 ? "text-emerald-400 border-emerald-500/30" :
                          Math.abs(move.movement) > 1 ? "text-yellow-400 border-yellow-500/30" :
        ...
        {!data && !lineMutation.isPending && (
          <Card className="bg-gray-900 border-gray-800">
            <CardContent className="p-12 text-center">
              <Activity className="h-12 w-12 text-cyan-400 mx-auto mb-4 opacity-50" />
