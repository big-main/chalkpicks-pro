...
function ResultBadge({ result }: { result: string }) {
  if (result === "win")
    return (
      <span className="badge-win px-2.5 py-0.5 text-[11px] font-bold tracking-wider">
        WIN
      </span>
    );
  if (result === "loss")
    return (
      <span className="badge-loss px-2.5 py-0.5 text-[11px] font-bold tracking-wider">
        LOSS
      </span>
    );
  return (
    <span className="badge-push px-2.5 py-0.5 text-[11px] font-bold tracking-wider">
      PUSH
    </span>
  );
}
...
export default function Performance() {
  ...
  const { data: recent, isLoading: recentLoading } =
    trpc.picks.recentSettled.useQuery(
      { limit: 15 },
      {
        refetchInterval: 30_000,
        staleTime: 0,
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative pt-28 pb-16 overflow-hidden">
        ...
        <div className="container relative z-10 text-center">
          ...
          >
            Our <span className="text-emerald-gradient">Performance</span>{" "}
            Record
          </h1>
          <p className="text-lg text-white/45 max-w-xl mx-auto mb-8">
            Every pick tracked. Every result verified. No cherry-picking — this
            is the full record.
          </p>
      ...
      <div className="container pb-20">
        ...
        {perfLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-12">
            <StatCard
              label="Win Rate"
              value={perf ? `${perf.overall.winRate}%` : "—"}
            ...
            <StatCard
              label="Total Picks"
              value={perf ? perf.overall.totalPicks.toLocaleString() : "—"}
              sub="Generated"
              color="#f0b800"
              icon={TrendingUp}
            />
            <StatCard
              label="Wins"
              value={perf ? perf.overall.wins.toLocaleString() : "—"}
              color="#39ff14"
              icon={Trophy}
            />
            <StatCard
              label="Losses"
              value={perf ? perf.overall.losses.toLocaleString() : "—"}
              color="#ff3b3b"
              icon={Target}
            />
            <StatCard
              ...
              value={
                perf
                  ? perf.overall.currentStreak > 0
                    ? `${perf.overall.currentStreak}W`
                    : perf.overall.currentStreak < 0
                      ? `${Math.abs(perf.overall.currentStreak)}L`
                      : "—"
                  : "—"
              }
              sub="Active"
            ...
            <StatCard
              ...
              value={
                perf
                  ? `${perf.overall.roi >= 0 ? "+" : ""}${perf.overall.roi}%`
                  : "—"
              }
              sub="$100 flat bet"
              color="#a855f7"
            ...
            <div className="space-y-4">
              {(perf?.byPickType ?? []).map(pt => (
                <div key={pt.type} className="flex items-center gap-3">
                  <div className="w-24 text-sm text-white/60 flex-shrink-0">
                    {pt.type}
                  </div>
                  <div className="flex-1 h-2 rounded-full overflow-hidden bg-white/5">
                    <div
                      ...
                      style={{
                        width: `${pt.winRate}%`,
                        background: PICK_TYPE_COLORS[pt.type] ?? "#39ff14",
                      }}
                    />
                  </div>
                  <div
                    className="w-12 text-right text-sm font-bold"
                    style={{ color: PICK_TYPE_COLORS[pt.type] ?? "#39ff14" }}
                  >
                    {pt.winRate}%
                  </div>
              ...
              <PieChart>
                ...
                >
                  {pieData.map((_, i) => (
                    <Cell
                      key={i}
                      fill={Object.values(PICK_TYPE_COLORS)[i % 4]}
                    />
                  ))}
                </Pie>
                <Tooltip
                ...
                <thead>
                  <tr className="border-b border-white/5">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-3 py-2.5 text-left text-[11px] font-semibold tracking-wider uppercase text-white/35"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {(recent?.picks ?? []).map((pick, i) => (
                    ...
                    >
                      ...
                      <td
                        className="px-3 py-3 data-table"
                        style={{
                          color:
                            (pick.odds ?? 0) > 0
                              ? "var(--accent-mint)"
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-2">
                          <ConfidenceBar
                            score={pick.confidenceScore ?? 0}
                            showLabel={false}
                            height="h-1.5"
                            className="w-10"
                          />
                          <span
                            className="data-table text-xs font-bold"
                            style={{
                              color:
                                (pick.confidenceScore ?? 0) >= 80
                                  ? "var(--accent-cyan)"
                                  : "#f0b800",
                            }}
                          >
              ...
              <div>
                ...
                <ul className="space-y-2.5 text-sm text-white/50">
                  <li className="flex items-start gap-2">
                    ...
                    Picks are logged with a timestamp before game start — no
                    retroactive edits
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-[#39ff14] mt-0.5 flex-shrink-0" />{" "}
                    Results are auto-graded against the closing line from the
                    originating sportsbook
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-[#39ff14] mt-0.5 flex-shrink-0" />{" "}
                    Pushes count as no-action (excluded from W/L record)
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-[#39ff14] mt-0.5 flex-shrink-0" />{" "}
                    ROI calculated on flat 1-unit sizing unless Kelly is
                    specified
                  </li>
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-2.5 text-sm text-white/50">
                  <li className="flex items-start gap-2">
                    ...
                    Full pick history available to all members (free and paid)
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-[#60a5fa] mt-0.5 flex-shrink-0" />{" "}
                    CLV (Closing Line Value) tracked on every bet — the #1
                    predictor of long-term edge
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-[#60a5fa] mt-0.5 flex-shrink-0" />{" "}
                    Sport-by-sport breakdown with separate W/L records per sport
                    and bet type
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-[#60a5fa] mt-0.5 flex-shrink-0" />{" "}
                    No cherry-picked timeframes — lifetime record shown by
                    default
                  </li>
                </ul>
              </div>
        ...
        >
          ...
          <p className="text-white/45 mb-8 max-w-md mx-auto">
            Get access to every pick, real-time odds, +EV finder, and all
            premium tools.
          </p>
