...
export default function Stats() {
  ...
  const { data: liveGames, isLoading: gamesLoading } =
    trpc.stats.liveGames.useQuery(
      { sportKey: sport },
      { enabled: hasPremiumAccess }
    );
  const { data: players, isLoading: playersLoading } =
    trpc.stats.topPlayers.useQuery(
      { sportKey: sport, limit: 12 },
      { enabled: hasPremiumAccess }
    );
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="page-top">
            ...
            <div className="container py-10">
              <div className="mb-6">
                <h1 className="font-display text-4xl tracking-wider">
                  PLATFORM <span className="text-gold-gradient">STATS</span>
                </h1>
                <p className="text-muted-foreground text-sm mt-1">
                  Real numbers from our public pick ledger — no estimates
                </p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
                {[
                  {
                    label: "Total Picks",
                    value: platformStats?.totalPicks ?? "—",
                    icon: "🎯",
                  },
                  {
                    label: "Win Rate",
                    value: platformStats?.winRate ?? "—",
                    icon: "📈",
                  },
                  {
                    label: "Ledger Entries",
                    value: platformStats?.ledgerEntries ?? "—",
                    icon: "🔒",
                  },
                  {
                    label: "Members",
              ...
              {platformStats?.sportStats &&
                platformStats.sportStats.length > 0 && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                    {platformStats.sportStats.map((s: any) => (
                      <div
                        key={s.label}
              ...
              <div className="text-xs text-muted-foreground">
                All picks are hash-locked before game time.{" "}
          ...
          <Paywall
            tier="daily"
            title="Live Stats"
            description="Real-time game scores, odds, and player statistics"
          />
        </div>
      </div>
    );
          ...
          <div className="container py-8">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                ...
                <p className="text-muted-foreground text-sm mt-1">
                  Real-time player stats, team performance, and matchup data
                </p>
              </div>
            ...
            {[
              {
                label: "Live Games",
                value:
                  liveGames?.filter((g: any) => g.status === "live").length ??
                  0,
                sub: "In progress",
              },
              {
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <Badge
                                className={`text-xs flex-shrink-0 border-0 capitalize ${
                            ...
                            {player.stats && (
                              <div className="mt-2 grid grid-cols-3 gap-1">
                                {Object.entries(
                                  player.stats as Record<string, any>
                                )
                                  .slice(0, 3)
                                  .map(([k, v]) => (
                                    <div
                                      key={k}
                                      className="text-center bg-secondary/50 rounded p-1"
                                    >
                            ...
                            <div className="text-center">
                              ...
                              {game.status === "live" && (
                                <div className="text-xs text-accent font-medium mt-0.5">
                                  {(game.quarter ?? game.period ?? game.minute)
                                    ? `${game.quarter ?? game.period ?? game.minute + "'"} ${game.timeLeft ?? ""}`
                                    : "LIVE"}
                                </div>
                              )}
              ...
              <div className="space-y-3">
                {injuries?.length === 0 && (
                  <div className="text-center py-12 text-muted-foreground">
                    No injury reports for this sport.
                  </div>
                )}
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                      <BarChart data={teamTrends}>
                        ...
                        <Tooltip
                          ...
                          formatter={(v: number) => [`${v}%`, "Over Rate"]}
                        />
                        <Bar
                          dataKey="overRate"
                          fill="oklch(0.72 0.19 162)"
                          radius={[4, 4, 0, 0]}
                          name="Over %"
                        />
                      </BarChart>
