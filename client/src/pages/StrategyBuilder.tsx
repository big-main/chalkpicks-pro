...
const FAQS = [
  ...
  {
    question:
      "What is the Kelly Criterion and how does it relate to bankroll %?",
    answer:
...
export default function StrategyBuilder() {
  ...
  const analyzeMutation = trpc.strategy.analyze.useMutation();
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const res = await analyzeMutation.mutateAsync({
        ...
        bankrollPct: bankrollPct[0],
        minEdge: minEdge[0],
      });
      setResult(res as unknown as StrategyResult);
    } catch {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative z-10 container pt-28 pb-20 max-w-6xl">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                Define your rules. Let AI project your edge.
              </p>
            </div>
        ...
        {!isSubscribed && (
          <NeonCard className="mb-8 p-6 text-center border-amber-500/30 bg-amber-500/5">
            ...
            <p className="text-muted-foreground mb-4 max-w-md mx-auto">
              The AI Strategy Builder is available to Pro and Elite subscribers.
              Upgrade to build, backtest, and save custom betting strategies.
            </p>
        ...
        <div
          className={`grid lg:grid-cols-2 gap-8 ${!isSubscribed ? "opacity-50 pointer-events-none select-none" : ""}`}
        >
          {/* Strategy Configuration */}
          <div className="space-y-6">
            <NeonCard className="p-6">
              <h2 className="text-lg font-bold text-foreground mb-5 flex items-center gap-2">
                <Target className="w-5 h-5 text-[#39ff14]" /> Strategy
                Configuration
              </h2>
            ...
            <NeonCard className="p-5">
              ...
              <div className="space-y-3">
                {[
                  {
                    step: "1",
                    text: "Define your strategy rules (sport, bet type, confidence, edge)",
                  },
                  {
                    step: "2",
                    text: "AI analyzes historical picks matching your criteria",
                  },
                  {
                    step: "3",
                    text: "Get projected ROI, win rate, and risk assessment",
                  },
                  { step: "4", text: "Backtest against real historical data" },
                  {
                    step: "5",
                    text: "Save and track your strategy's live performance",
                  },
                ].map(({ step, text }) => (
          ...
          <div className="space-y-6">
            {!result && !isGenerating && (
              <NeonCard className="p-8 text-center border-dashed">
                ...
                <p className="text-muted-foreground text-sm">
                  Configure your strategy and click Generate to see AI
                  projections
                </p>
              </NeonCard>
            )}
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground text-sm mt-1">
                      Scanning historical picks and projecting performance
                    </p>
                  </div>
                </div>
              </NeonCard>
              ...
              <>
                ...
                <NeonCard className="p-6 border-[#39ff14]/20">
                  <div className="flex items-center justify-between mb-5">
                    <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
                      <BarChart3 className="w-5 h-5 text-[#39ff14]" /> Strategy
                      Analysis
                    </h2>
                    <Badge
                      className={`text-xs border ${riskColor(result.riskLevel)}`}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-5">
                    <div className="p-4 rounded-lg bg-[#39ff14]/5 border border-[#39ff14]/15">
                      ...
                      <p
                        className={`text-2xl font-bold ${result.projectedROI >= 0 ? "text-[#39ff14]" : "text-red-400"}`}
                      >
                        {result.projectedROI > 0 ? "+" : ""}
                        {result.projectedROI}%
                      </p>
                    </div>
                    ...
                    <div className="p-4 rounded-lg bg-amber-500/5 border border-amber-500/15">
                      ...
                      <p className="text-2xl font-bold text-amber-400">
                        ${result.expectedMonthlyProfit.toFixed(0)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 text-sm">
                    {[
                      ...
                      {
                        label: "Bankroll Per Bet",
                        value: `${result.bankrollPct}%`,
                      },
                      {
                        label: "Min Edge Required",
                        value: `${result.minEdge}%`,
                      },
                    ].map(({ label, value }) => (
                      <div
                        key={label}
                ...
                <NeonCard className="p-6">
                  <h3 className="text-base font-bold text-foreground mb-4 flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-blue-400" /> Historical
                    Backtest
                  </h3>
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div>
                      ...
                      <p
                        className={`text-xl font-bold ${result.backtestROI >= 0 ? "text-[#39ff14]" : "text-red-400"}`}
                      >
                        {result.backtestROI > 0 ? "+" : ""}
                        {result.backtestROI}%
                      </p>
                    </div>
                    <div>
