...
**Root cause:** `www.chalkpicks.pro` pages redirect 301 → `chalkpicks.pro` equivalents.
This is CORRECT behavior (www → non-www canonical). Google reports these as "not indexed" because they redirect.
**Fix:** No code fix needed — this is expected. Google will eventually stop reporting these. But ensure `www.chalkpicks.pro` is NOT in the sitemap (it isn't — confirmed).
