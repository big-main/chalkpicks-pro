...
2. **HTTP Request** → GET `https://chalkpicks.pro/api/trpc/picks.list` (fetch today's picks)
...
```json
{
  ...
  "nodes": [
    ...
    {
      "parameters": {
        "url": "https://chalkpicks.pro/api/trpc/picks.list?input={\"json\":{\"limit\":3}}",
        "method": "GET",
        "responseFormat": "json"
      },
  ...
  "connections": {
    "Schedule Trigger": {
      "main": [[{ "node": "Fetch Today Picks", "type": "main", "index": 0 }]]
    },
    "Fetch Today Picks": {
      "main": [[{ "node": "Format Posts", "type": "main", "index": 0 }]]
    },
    "Format Posts": {
