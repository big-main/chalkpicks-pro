...
describe("Discord Steam Alerts Webhook Configuration", () => {
  ...
  it("DISCORD_STEAM_WEBHOOK_URL should have webhook ID and token", () => {
    ...
    expect(parts?.[0]?.length, "Webhook ID should be numeric").toBeGreaterThan(
      0
    );
    expect(
      parts?.[1]?.length,
  ...
  it("Both Discord webhook URLs should be set", () => {
    ...
    expect(steamWebhook, "DISCORD_STEAM_WEBHOOK_URL must be set").toBeTruthy();
  });
});
