...
export const adminRouter = router({
  ...
  getAllApplications: adminProcedure
    .input(
      z.object({
        status: z
          .enum(["all", "pending", "approved", "rejected", "not_applied"])
          .optional()
          .default("all"),
  ...
  approveApplication: adminProcedure
    .input(
      z.object({
        userId: z.number(),
        accessTier: z
          .enum(["recreational", "serious", "professional"])
          .optional(),
      })
    )
  ...
  updateUserTier: adminProcedure
    .input(
      z.object({
        userId: z.number(),
        subscriptionTier: z
          .enum(["free", "trial", "daily", "monthly", "yearly"])
          .optional(),
        accessTier: z
          .enum(["free", "recreational", "serious", "professional"])
          .optional(),
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.subscriptionTier)
        updates.subscriptionTier = input.subscriptionTier;
  ...
  // ─── Odds API Cache Controls ──────────────────────────────────────────────

  /** Get cache statistics */
  getCacheStats: adminProcedure.query(async () => {
    return oddsApiCache.getStats();
  }),

  /** Purge all cached odds data */
  purgeAllCache: adminProcedure.mutation(async () => {
    await oddsApiCache.purgeAll();
    return { success: true, message: "All odds cache purged" };
  }),

  /** Purge cache for a specific sport */
  purgeSportCache: adminProcedure
    .input(z.object({ sport: z.string() }))
    .mutation(async ({ input }) => {
      await oddsApiCache.purgeSport(input.sport);
      return { success: true, message: `Cache purged for ${input.sport}` };
    }),

  /** Reset quota counter (for new billing month) */
