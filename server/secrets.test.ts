...
describe("Secrets Configuration", () => {
  it.skipIf(!process.env.SMTP_USER)("should have SMTP credentials configured", () => {
  ...
  it.skipIf(!process.env.ODDS_API_IO_KEY)("should have Odds API IO key configured", () => {
    expect(process.env.ODDS_API_IO_KEY).toBeDefined();
    expect(process.env.ODDS_API_IO_KEY?.length).toBeGreaterThan(10);
  });

  it.skipIf(!process.env.SMTP_USER)("should validate SMTP credentials format", () => {
    const smtpUser = process.env.SMTP_USER;
    expect(smtpUser).toMatch(/^[^\s@]+@[^\s@]+\.[^\s@]+$/);
  });

  it.skipIf(!process.env.ODDS_API_IO_KEY)("should validate Odds API IO key format (hex string)", () => {
